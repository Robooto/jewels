...
Jewels.prototype.addJewel = function (i, j) {
    ...
    // Init the jewel // 30 and 70 // added the + 20 and 10
	...
	this.game.add.tween(jewel.scale).to({x: 1, y: 1}, 200).start();
};
	...
	for (var i = 0; i < gridSizeX; i++) {
        for (var j = 0; j < gridSizeY; j++) {
			...
			if (jewel.selected) {
                ...
                this.game.add.tween(jewel.scale).to({x: 0, y: 0}, 200).start();
                jewel.alive = false;
			}
		}
	}
...
Jewels.prototype.moveJewelsDown = function(gridSizeX, gridSizeY) {
    ...
    for (var j = gridSizeY-1; j >= 0; j--) {
            ...
            if (!jewel) {
                ...
                this.game.add.tween(jewel).to({y: jewel.y + moveBy*(jewel.height+10)}, 300).start();
            }
        }
    }
};
